...
const getToken = () => new Promise((resolve, reject) => {
  if (!token) {
    vm.fetch.get(`${ROOT}/qiniu`).then((t) => {
      token = t
      resolve(token)
    }).catch((e) => reject(e))
  } else {
...
// 七牛上传实现
export const upload = ({file}, onUploadProgress = () => {}) => getToken().then((token) => {
  ...
  return vm.fetch.post(UPLOAD_URL, form, {headers: {'Content-Type': 'multipart/form-data'}, onUploadProgress})
})
...
//       form.append('file', file)
//       console.log('form:',form)
//       return vm.fetch.post(UPLOAD_URL, form, {headers: {'Content-Type': 'multipart/form-data'}})
//   })
// }
